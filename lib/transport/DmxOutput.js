...
export default class DmxOutput {
   ...
   *
   * @param {{send:function(Buffer)}} driver The driver that will bring the
  ...
  /**
   * Sends the Buffer to the driver. Should only be called this manually it
   * no interval was specified to the constructor.
   */
  send() {
  ...
  /**
   * Starts the timer sending the data-frames.
  ...
  /**
   * Stops the timer sending data-frames. Can always be started again by
   * calling start().
   */
  stop() {
    if (this.timer) {
